...
contract InternalBribe is IBribe {
    ...
    */
    ...
    function rewardPerToken(address token) public view returns (uint) {
        ...
        return rewardPerTokenStored[token] + ((lastTimeRewardApplicable(token) - Math_0.min(lastUpdateTime[token], periodFinish[token])) * rewardRate[token] * PRECISION / totalSupply);
    }
    ...
    function earned(address token, uint tokenId) public view returns (uint) {
        uint _startTimestamp = Math_0.max(lastEarn[token][tokenId], rewardPerTokenCheckpoints[token][0].timestamp);
        ...
        reward += cp.balanceOf * (rewardPerToken(token) - Math_0.max(_rewardPerTokenStored, userRewardPerTokenStored[token][tokenId])) / PRECISION;
