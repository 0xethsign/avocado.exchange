...
contract VeArtProxy is IVeArtProxy {
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        string memory json = Base64_0.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "Avocado locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64_0.encode(bytes(output)), '"}'))));
