...
contract Gauge is IGauge {
    ...
    */
    ...
    function lastTimeRewardApplicable(address token) public view returns (uint) {
        return Math_0.min(block.timestamp, periodFinish[token]);
    }
    ...
    function rewardPerToken(address token) public view returns (uint) {
        ...
        return rewardPerTokenStored[token] + ((lastTimeRewardApplicable(token) - Math_0.min(lastUpdateTime[token], periodFinish[token])) * rewardRate[token] * PRECISION / derivedSupply);
    }
    ...
    function _batchRewardPerToken(address token, uint maxRuns) internal returns (uint, uint) {
        ...
        uint _endIndex = Math_0.min(supplyNumCheckpoints-1, maxRuns);
    ...
    function _calcRewardPerToken(address token, uint timestamp1, uint timestamp0, uint supply, uint startTimestamp) internal view returns (uint, uint) {
        uint endTime = Math_0.max(timestamp1, startTimestamp);
        return (((Math_0.min(endTime, periodFinish[token]) - Math_0.min(Math_0.max(timestamp0, startTimestamp), periodFinish[token])) * rewardRate[token] * PRECISION / supply), endTime);
    }
    ...
    function _updateRewardPerToken(address token, uint maxRuns, bool actualLast) internal returns (uint, uint) {
        ...
        if (actualLast) {
            ...
            if (sp.supply > 0) {
                (uint _reward,) = _calcRewardPerToken(token, lastTimeRewardApplicable(token), Math_0.max(sp.timestamp, _startTimestamp), sp.supply, _startTimestamp);
                reward += _reward;
    ...
    function earned(address token, address account) public view returns (uint) {
        uint _startTimestamp = Math_0.max(lastEarn[token][account], rewardPerTokenCheckpoints[token][0].timestamp);
        ...
        reward += cp.balanceOf * (rewardPerToken(token) - Math_0.max(_rewardPerTokenStored, userRewardPerTokenStored[token][account])) / PRECISION;
