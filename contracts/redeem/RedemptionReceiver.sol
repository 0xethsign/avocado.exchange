...
import "contracts/interfaces/IAvo.sol";

/// @notice Part 2 of 2 in the WeVE (FTM) -> USDC + AVO (OP) redemption process
/// This contract is responsible for receiving the LZ message and distributing USDC + AVO
contract RedemptionReceiver is ILayerZeroReceiver {
    IERC20 public immutable USDC;
    IAvo public immutable AVO;
    ...
    uint256 public redeemableAVO;
    uint256 public leftoverAVO;
    ...
    event Initialized(address fantomSender, uint256 redeemableUSDC, uint256 redeemableAVO);

    function initializeReceiverWith(
        address _fantomSender,
        uint256 _redeemableUSDC,
        uint256 _redeemableAVO
    ) external onlyTeam {
        ...
        redeemableAVO = _redeemableAVO;
        leftoverAVO = _redeemableAVO;

        emit Initialized(fantomSender, redeemableUSDC, redeemableAVO);
    }
    ...
    function previewRedeem(uint256 amountWEVE)
        public
        view
        returns (uint256 shareOfUSDC, uint256 shareOfAVO)
    {
        // pro rata USDC
        shareOfUSDC = (amountWEVE * redeemableUSDC) / ELIGIBLE_WEVE;
        // pro rata AVO
        shareOfAVO = (amountWEVE * redeemableAVO) / ELIGIBLE_WEVE;
    }
    ...
    function lzReceive(
        ...
        (uint256 shareOfUSDC, uint256 shareOfAVO) = previewRedeem(amountWEVE);
        ...
        leftoverAVO -= shareOfAVO; // this will revert if underflows
        require(
            AVO.claim(redemptionAddress, shareOfAVO),
            "CLAIM_FAILED"
        );
    }
