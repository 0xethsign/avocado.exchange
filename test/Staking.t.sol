...
contract StakingTest is BaseTest {
    ...
    function deployBaseCoins() public {
        ...
        escrow = new TestVotingEscrow(address(AVO));
        voter = new TestVoter();
    }
    ...
    function createLock2() public {
        ...
        owner2.approve(address(AVO), address(escrow), TOKEN_1);
    ...
    function createLock3() public {
        ...
        owner3.approve(address(AVO), address(escrow), TOKEN_1);
    ...
    function deployFactory() public {
        ...
        staking = new TestStakingRewards(address(stake), address(AVO));
    }
    ...
    function depositEmpty() public {
        ...
        assertEq(gauge.earned(address(AVO), address(owner)), staking.earned(address(owner)));
    }
    ...
    function depositEmpty2() public {
        ...
        assertEq(gauge.earned(address(AVO), address(owner2)), staking.earned(address(owner2)));
    }
    ...
    function depositEmpty3() public {
        ...
        assertEq(gauge.earned(address(AVO), address(owner3)), staking.earned(address(owner3)));
    }

    function notifyRewardsAndCompare() public {
        depositEmpty3();

        AVO.approve(address(staking), TOKEN_1M);
        AVO.approve(address(gauge), TOKEN_1M);
        assertEq(staking.rewardPerTokenStored(), gauge.rewardPerTokenStored(address(AVO)));
        staking.notifyRewardAmount(TOKEN_1M);
        gauge.notifyRewardAmount(address(AVO), TOKEN_1M);
    ...
    function notifyRewardsAndCompareOwner1Again() public {
        ...
        gauge.batchRewardPerToken(address(AVO), 200);
        assertEq(staking.rewardPerTokenStored(), gauge.rewardPerTokenStored(address(AVO)));
        // uint256 sb = AVO.balanceOf(address(owner));
        staking.getReward();
        // uint256 sa = AVO.balanceOf(address(owner));
        // uint256 gb = AVO.balanceOf(address(owner));
        address[] memory tokens = new address[](1);
        tokens[0] = address(AVO);
        gauge.getReward(address(owner), tokens);
        // uint256 ga = AVO.balanceOf(address(owner));
        vm.warp(block.timestamp + 604800);
        vm.roll(block.number + 1);
        staking.withdraw(1e21);
        gauge.withdraw(1e21);
