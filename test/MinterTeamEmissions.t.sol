...
contract MinterTeamEmissions is BaseTest {
    ...
    function setUp() public {
        ...
        deployCoins();
        mintStables();
        uint256[] memory amountsAvo = new uint256[](2);
        amountsAvo[0] = 1e25;
        amountsAvo[1] = 1e25;
        mintAvo(owners, amountsAvo);
        team = new TestOwner();
        ...
        AVO.approve(address(router), TOKEN_1);
        ...
        address pair = router.pairFor(address(FRAX), address(AVO), false);

        AVO.approve(address(voter), 5 * TOKEN_100K);
        ...
        assertEq(AVO.balanceOf(address(minter)), 14 * TOKEN_1M);
        ...
        vm.roll(block.number + 1);
        before = AVO.balanceOf(address(owner));
        minter.update_period(); // initial period week 2
        after_ = AVO.balanceOf(address(owner));
    ...
    function testTeamEmissionsRate() public {
        ...
        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        uint256 beforeTeamSupply = AVO.balanceOf(address(team));
        uint256 weekly = minter.weekly_emission();
        uint256 growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
        uint256 afterTeamSupply = AVO.balanceOf(address(team));
        uint256 newTeamAvo = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamAvo) * 30) / 1000, newTeamAvo); // check 3% of new emissions to team

        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        beforeTeamSupply = AVO.balanceOf(address(team));
        weekly = minter.weekly_emission();
        growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
        afterTeamSupply = AVO.balanceOf(address(team));
        newTeamAvo = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamAvo) * 30) / 1000, newTeamAvo); // check 3% of new emissions to team

        // rate is right even when AVO is sent to Minter contract
        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        owner2.transfer(address(AVO), address(minter), 1e25);
        beforeTeamSupply = AVO.balanceOf(address(team));
        weekly = minter.weekly_emission();
        growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
    ...
    function testChangeTeamEmissionsRate() public {
        ...
        uint256 afterTeamSupply = AVO.balanceOf(address(team));
        uint256 newTeamAvo = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamAvo) * 50) / 1000, newTeamAvo); // check 5% of new emissions to team
    }
}
