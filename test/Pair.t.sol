...
contract PairTest is BaseTest {
    ...
    function deployPairCoins() public {
        ...
        amounts[1] = 1e25;
        amounts[2] = 1e25;
        mintAvo(owners, amounts);
        mintLR(owners, amounts);

        VeArtProxy artProxy = new VeArtProxy();
        escrow = new VotingEscrow(address(AVO), address(artProxy));
    }

    function createLock() public {
        deployPairCoins();

        AVO.approve(address(escrow), 5e17);
        escrow.create_lock(5e17, 4 * 365 * 86400);
        vm.roll(block.number + 1); // fwd 1 block because escrow.balanceOfNFT() returns 0 in same block
        assertGt(escrow.balanceOfNFT(1), 495063075414519385);
        assertEq(AVO.balanceOf(address(escrow)), 5e17);
    }
    ...
    function increaseLock() public {
        ...
        assertEq(AVO.balanceOf(address(escrow)), TOKEN_1);
    }
    ...
    function votingEscrowMerge() public {
        ...
        AVO.approve(address(escrow), TOKEN_1);
        escrow.create_lock(TOKEN_1, 4 * 365 * 86400);
        assertGt(escrow.balanceOfNFT(2), 995063075414519385);
        assertEq(AVO.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(AVO.balanceOf(address(escrow)), 3 * TOKEN_1);
        console2.log(escrow.totalSupply());
        escrow.merge(3, 1);
        console2.log(escrow.totalSupply());
        assertGt(escrow.balanceOfNFT(1), 1990063075414519385);
    ...
    function deployMinter() public {
        ...
        AVO.setMinter(address(minter));
        address[] memory tokens = new address[](5);
        tokens[0] = address(USDC);
        tokens[1] = address(FRAX);
        tokens[2] = address(DAI);
        tokens[3] = address(AVO);
        tokens[4] = address(LR);
    ...
    function deployPairFactoryGauge() public {
        ...
        AVO.approve(address(gaugeFactory), 15 * TOKEN_100K);
        ...
        staking = new TestStakingRewards(address(pair), address(AVO));
    ...
    function addGaugeAndBribeRewards() public {
        ...
        AVO.approve(address(gauge), PAIR_1);
        AVO.approve(address(xbribe), PAIR_1);
        AVO.approve(address(staking), PAIR_1);

        gauge.notifyRewardAmount(address(AVO), PAIR_1);
        xbribe.notifyRewardAmount(address(AVO), PAIR_1);
        staking.notifyRewardAmount(PAIR_1);

        assertEq(gauge.rewardRate(address(AVO)), 1653);
        // no reward rate, all or nothing
        // assertEq(xbribe.rewardRate(address(AVO)), 1653);
    ...
    function createLock2() public {
        ...
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(AVO.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        AVO.approve(address(voter), PAIR_1);
    ...
    function minterMint() public {
        ...
        console2.log(AVO.balanceOf(address(distributor)));
        console2.log(distributor.claimable(1));
        uint256 claimable = voter.claimable(address(gauge));
        AVO.approve(address(staking), claimable);
    ...
    function gaugeClaimRewards() public {
        ...
        staking.getReward();
        vm.warp(block.timestamp + 1);
        uint256 before = AVO.balanceOf(address(owner));
        vm.warp(block.timestamp + 1);
        gauge.batchRewardPerToken(address(AVO), 200);
        vm.warp(block.timestamp + 1);
        gauge.batchRewardPerToken(address(AVO), 200);
        vm.warp(block.timestamp + 1);
        gauge.batchRewardPerToken(address(AVO), 200);
        ...
        gauge.batchRewardPerToken(address(AVO), 200);
        vm.warp(block.timestamp + 1);
        uint256 earned = gauge.earned(address(AVO), address(owner));
        address[] memory rewards = new address[](1);
        rewards[0] = address(AVO);
        vm.warp(block.timestamp + 1);
        gauge.getReward(address(owner), rewards);
        vm.warp(block.timestamp + 1);
        uint256 after_ = AVO.balanceOf(address(owner));
    ...
    function minterMint2() public {
        ...
        assertEq(gauge.rewardRate(address(AVO)), staking.rewardRate());
        console2.log(gauge.rewardPerTokenStored(address(AVO)));
    }
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pair), address(gauge), PAIR_1);
        owner3.deposit(address(gauge), PAIR_1, 0);
        uint256 before = AVO.balanceOf(address(owner3));
        vm.warp(block.timestamp + 1);
        // uint256 earned = gauge.earned(address(AVO), address(owner3));
        address[] memory rewards = new address[](1);
        rewards[0] = address(AVO);
        owner3.getGaugeReward(address(gauge), address(owner3), rewards);
        uint256 after_ = AVO.balanceOf(address(owner3));
        uint256 received = after_ - before;
        assertGt(received, 0);
        console2.log(gauge.rewardPerTokenStored(address(AVO)));
    ...
    function testGaugeClaimRewards3() public {
        ...
        pair.approve(address(gauge), PAIR_1);
        gauge.deposit(PAIR_1, 0);
        AVO.approve(address(gauge), AVO.balanceOf(address(owner)));
        gauge.notifyRewardAmount(address(AVO), AVO.balanceOf(address(owner)));
