...
import "contracts/Avo.sol";
import "contracts/AvocadoLibrary.sol";
...
abstract contract BaseTest is Test, TestOwner {
    ...
    function mintAvo(address[] memory _accounts, uint256[] memory _amounts) public {
        for (uint256 i = 0; i < _amounts.length; i++) {
            AVO.mint(_accounts[i], _amounts[i]);
        }
    }
    ...
    function deployPairFactoryAndRouter() public {
        ...
        lib = new AvocadoLibrary(address(router));
    }
