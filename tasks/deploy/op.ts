...
task("deploy:op", "Deploys Optimism contracts").setAction(async function (
  ...
  const [
    ...
    ethers.getContractFactory("AvocadoLibrary"),
    ...
    ethers.getContractFactory("AvoGovernor"),
  ...
  await avo.deployed();
  console.log("Avo deployed to: ", avo.address);
  ...
  console.log("AvocadoLibrary deployed to: ", library.address);
  ...
  const escrow = await VotingEscrow.deploy(avo.address, artProxy.address);
  await escrow.deployed();
  console.log("VotingEscrow deployed to: ", escrow.address);
  console.log("Args: ", avo.address, artProxy.address, "\n");
  ...
  const governor = await AvoGovernor.deploy(escrow.address);
  await governor.deployed();
  console.log("AvoGovernor deployed to: ", governor.address);
  console.log("Args: ", escrow.address, "\n");

  // Airdrop
  const claim = await MerkleClaim.deploy(avo.address, OP_CONFIG.merkleRoot);
  await claim.deployed();
  console.log("MerkleClaim deployed to: ", claim.address);
  console.log("Args: ", avo.address, OP_CONFIG.merkleRoot, "\n");

  // Initialize
  await avo.initialMint(OP_CONFIG.teamEOA);
  console.log("Initial minted");

  await avo.setRedemptionReceiver(receiver.address);
  console.log("RedemptionReceiver set");

  await avo.setMerkleClaim(claim.address);
  console.log("MerkleClaim set");

  await avo.setMinter(minter.address);
